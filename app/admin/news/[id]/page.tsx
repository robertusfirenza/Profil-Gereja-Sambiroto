  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-semibold text-foreground">Edit Berita</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-3xl rounded-2xl border border-border bg-card p-8">
          ...
          <div>
            ...
            {(imagePreview || form.image) ? (
              <div className="relative inline-block">
                ...
                <button type="button" onClick={() => { setImagePreview(""); setForm((p) => ({ ...p, image: "" })) }} className="absolute -right-2 -top-2 rounded-full bg-red-500 p-1 text-white">
                  <X className="h-3 w-3" />
                </button>
              </div>
            ) : (
              <button type="button" onClick={() => fileRef.current?.click()} className="flex h-32 w-full items-center justify-center rounded-lg border-2 border-dashed border-border text-sm text-muted-foreground hover:border-foreground hover:text-foreground transition-colors">
        ...
        <div className="mt-8 flex items-center justify-end gap-3">
          ...
          <button type="submit" disabled={saving} className="rounded-lg bg-foreground px-6 py-2 text-sm font-medium text-background hover:opacity-90 disabled:opacity-50">{saving ? "Menyimpan..." : "Simpan"}</button>
        </div>
      </form>
    </>
  )
